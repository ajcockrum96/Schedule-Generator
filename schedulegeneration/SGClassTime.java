 ...
 * </p>
 * <p>
 * Much like the SGClassInfo class, there are not many methods included in this
...
public class SGClassTime {
	...
	/**
	 * Constructs a new SGClassTime with the specified timeRange and name.
	 *
	 * @param timeRange		the SGTimeRange of the class time
	 * @param name			the String of the class name
	 */
	public SGClassTime(SGTimeRange timeRange, String name) {
		timePeriod = timeRange;
		className = name;
	}

	/**
	 * Constructs a new SGClassTime with the time period given by the times in
	 ...
	 * @param name			the String of the class name
	 */
	public SGClassTime(String rangeString, String days, String name) {
		this(new SGTimeRange(rangeString, days), name);
	}

	/**
	 * Searches an ArrayList of SGClassTime objects for an instance with a specific
	 ...
	 *
	 * @param classTimes	the ArrayList of the SGClassTime objects to sort
	 ...
	 */
	static public int searchForClassInArrayList(ArrayList<SGClassTime> classTimes, String name, int startIndex) {
	...
	/**
	 * Searches an entire ArrayList of SGClassTime objects for a SGClassTime with
	 ...
	 * @return				the index of the first found instance with the desired name
	 */
	static public int searchForClassInArrayList(ArrayList<SGClassTime> classTimes, String name) {
		return searchForClassInArrayList(classTimes, name, 0);
	}

	/**
	 * Merge sorts an ArrayList of SGClassTime Objects based on their timePeriod
	 * start times.
	 *
	 * @param classTimes	the ArrayList of SGClassTime objects to sort
	 ...
	 */
	static public void mergeSortSGClassTimeArrayList(ArrayList<SGClassTime> classTimes, int startIndex, int endIndex) {
			...
			if(length > 2) {
				...
				for(i = startIndex, j = midIndex; i < midIndex && j < endIndex;) {
					if(SGTimeRange.compareTimeRangeStarts(classTimes.get(i).timePeriod, classTimes.get(j).timePeriod) < 0) {
						SGClassTime temp = classTimes.remove(i);
						classTimes.add(startIndex++, temp);
						++i;
					}
					else {
						SGClassTime temp = classTimes.remove(j);
						classTimes.add(startIndex++, temp);
						++midIndex;
						++i;
						++j;
				...
				if(SGTimeRange.compareTimeRangeStarts(classTimes.get(startIndex).timePeriod, classTimes.get(startIndex + 1).timePeriod) > 0) {
					SGClassTime a = classTimes.get(startIndex);
					SGClassTime b = classTimes.get(startIndex + 1);
